...
*Я оставлю текст самого задания, чтоб потом случайные прохожие не думали, что я сумасшедший, который сам выдумал себе баги и потом фиксил их...*
...
Первым делом я нашёл файл pre-commit.script и переименовал его: ```mv .git/hooks/pre-commit.sample .git/hooks/pre-commit```. После сделал его исполняемым: ```chmod +x .git/hooks/pre-commit```. В сам файл я вставил такой код:
...
for file in $(git diff --cached --name-only --diff-filter=ACM | grep '\.txt$'); do
    if ! grep -q "Автор" "$file"; then
        echo "Ошибка: В файле $file отсутствует строка 'Автор'."
        echo "Пожалуйста, добавьте строку перед коммитом."
        exit 1
    fi
done
